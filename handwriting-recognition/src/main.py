...
link=StringVar()
Label(root, text='Paste Your Link Here', font='arial 15 bold').place(x=300,y=60)
link_enter=Entry(root, width=100, textvariable=link).place(x=92, y=90)
class FilePaths:
    """Filenames and paths to data."""
    fn_char_list = 'handwriting-recognition\\model\\charList.txt'
    fn_summary = 'handwriting-recognition\\model\\summary.json'
    fn_corpus = 'handwriting-recognition\\data\\corpus.txt'
...
def infer(model: Model, fn_img: Path) -> None:
    ...
    a=f'Recognized: "{recognized[0]}"'
    b=f'Probability: "{probability[0]}"'
    Label(root,text=a, font='arial 15 bold').place(x=190,y=220)
    Label(root,text=b, font='arial 15 bold').place(x=190,y=250)
...
def Recognize():
     ...
     main(img_file)
Button(root,text='Recognize', font='arial 15 bold', bg='pale violet red', padx=2,command=Recognize).place(x=330,y=150)
root.mainloop()